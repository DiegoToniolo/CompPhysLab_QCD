...
def Chi2g(g, par, x, y, err_y):
    sum = 0.0
    for i in range(0, len(x)):
        sum += ((g(x[i], par[0], par[1]) - y[i])/err_y[i])**2.0
    return sum
...
for j in range(0, n_t0 - 1):
    ...
    prob = 100 * (1-chi2.cdf(chi2_comp, len(x) - 2))
    print("Fit: Chi2 = {:.6f}; Prob = {:.2f}".format(chi2_comp, prob))
    print("b = {:.10f} +- {:.10f}; c = {:.10f} +- {:.10f}".format(par[0], npy.sqrt(par_err[0][0]), par[1], npy.sqrt(par_err[1][1])))
    plt.xlim(0, max(x) + 0.05)
    y_model = []
    for i in x_model:
        y_model += [g(i, par[0], par[1])]
    plt.xlabel("$a^2/t_0$")
